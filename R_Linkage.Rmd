...
All source code, and files used in the linkages are available on github at  [https://github.com/parrish-epi/R-recordLinkage](https://github.com/parrish-epi/R-recordLinkage)
...
knitr::kable(dat1a.dedup %>% 
             ...
             caption = "Table 1: First five records of aligned dataset 1 (dat1)") %>%
...
knitr::kable(dat2a %>% 
             arrange(FamilyName) %>%
             head(5), booktabs = TRUE,
             caption = "Table2: First five records of aligned dataset 2 (dat2)") %>%
       kableExtra::kable_styling(latex_options = c("scale_down", "HOLD_position"))
```
...
```{r echo=FALSE, fig.cap="Figure 1: General Data Linkage Flow Diagram", fig.alt="General data linkage flow diagram.", out.width = '100%'}
...
knitr::kable(MergedDat %>% 
             arrange(FamilyName) %>%
             head(5), booktabs = TRUE,
             caption = "Table 3: First five records of merged datasets") %>%
...
```{r echo=FALSE, fig.cap="Figure 2: Mean Residual Life (MRL) plot.",fig.alt="Image of Mean Residual Life (MRL) on y-axis, and threshold level.", out.width = '100%'}
knitr::include_graphics(
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, fig.cap="Figure 3: Histogram of counts by bin for possible matches.", fig.alt="Histogram of counts by bin for possible matches.",fig.width=10,fig.height=5}
...
```{r echo=FALSE, fig.cap="Figure 4: Image of potential matches from the r base editor.",fig.alt="Image of potential matches from the r base editor.", out.width = '100%'}
knitr::include_graphics(
...
Rev_pairs %>% filter(!is.na(is_match)) %>%
       ...
       caption = "Table 4: Cumulative linkage match % by bin"
)
